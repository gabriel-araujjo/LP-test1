...
int rSequentialSearch(T *vector, int size, T &element) {
    ...
    /* Recursão simples, não é de calda */
...
int rBinarySearch(T *vector, int size, T &element) {
    ...
    /* Recursão simples, apesar de haver duas chamadas a rBinarySearch, somente uma delas é chamada por vez,
     * por isso não se trata de recursividade de calda
     **/
